...
EOF
    ...
    (
        ...
        snap_src=$(
            grep "$regex" $STAT_POSTGRES_FILE | sed 's/database_stat/snap/')
        ...
        if [[ -z "$snap_src" ]]; then
            ...
            xact_rollback=$((
                ${stat['xact_rollback']} - ${snap['xact_rollback']} ))
            tup_fetched=$(( ${stat['tup_fetched']} - ${snap['tup_fetched']} ))
            tup_returned=$((
                ${stat['tup_returned']} - ${snap['tup_returned']} ))
            tup_inserted=$((
                ${stat['tup_inserted']} - ${snap['tup_inserted']} ))
...
EOF
    ...
    (
        ...
        snap_src=$(
            grep "$regex" $STAT_POSTGRES_FILE | sed 's/bgwriter_stat/snap/')
...
EOF
    ...
    (
        ...
        snap_src=$(
            grep "$regex" $STAT_POSTGRES_FILE | sed 's/tables_stat/snap/')
        ...
        if [[ -z "$snap_src" ]]; then
            ...
            n_tup_hot_upd=$((
                ${stat['n_tup_hot_upd']} - ${snap['n_tup_hot_upd']} ))
