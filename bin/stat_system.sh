...
(
    ...
    (
        ...
        if [[ -z "$snap_src" ]]; then
            ...
            total_int=$((
                ...
                $irq_int + $softirq_int + $steal_int + $guest_int +
                $guest_nice_int
            ))
    ...
    (
        ...
        if [[ -z "$snap_src" ]]; then
            ...
            swap_in_s=$((
                (${stat['swap_in']} - ${snap['swap_in']}) / $interval ))
            swap_out_s=$((
    ...
    part_list=$(
        lsblk -ro KNAME,TYPE | grep ' part' | sed 's/ .*//' | sort | uniq)
    ...
    for part in $part_list; do
        ...
        (
            ...
            if [[ -z "$snap_src" ]]; then
                ...
                read_merged_s=$((
                    (${stat['read_merged']} - ${snap['read_merged']}) /
                    $interval ))
                read_sectors_s=$((
