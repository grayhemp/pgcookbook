...
for dbname in $ARCHIVE_DBNAME_LIST; do
    ...
    part_list=$($PSQL -XAt -F '.' -c "$ARCHIVE_PARTS_SQL" $dbname 2>&1) || \
        die "Can not get a table list: $part_list."
    ...
    for part in $part_list; do
        ...
        if ! $ARCHIVE_DRY_RUN; then
            error=$($PGDUMP -F c -Z 2 -t $part \
                ...
                die "Can not dump table $part from $dbname: $error."
            ...
            error=$($PSQL -c "DROP TABLE $part;" $dbname 2>&1) || \
                die "Can not drop table $part from $dbname: $error."

            error=$(ARCHIVE_COMMAND_AFTER_DROP $dbname $part $file 2>&1) || \
                die "After-command error for $part from $dbname: $error."

            info "Table $part from $dbname dumped in $dbname/$file."
        else
            info "Table $part from $dbname can be dumped in $dbname/$file."
        fi
    done
...
info "Execution time, s: dump ${dump_time:-N/A}, sync ${sync_time:-N/A}."
