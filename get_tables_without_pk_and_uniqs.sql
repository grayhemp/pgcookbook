...
WHERE
    ...
    NOT EXISTS (
        SELECT pg_catalog.pg_index.indexrelid
        FROM pg_catalog.pg_index
        WHERE
            pg_catalog.pg_index.indrelid = c.oid AND
            pg_catalog.pg_index.indisunique='t' AND
            NOT EXISTS (
                ...
                JOIN pg_catalog.pg_attribute i_attr ON
                    ...
                    i_attr.attrelid = pg_catalog.pg_index.indexrelid
                WHERE
                    t_attr.attrelid = c.oid AND
                    t_attr.attnotnull <> 't'
            )
    )
ORDER BY 1, 2;
